...
# ==========================================
# 1. UI 디자인 최적화 (하이라이트 탭 & 일관된 배경)
# ==========================================
def inject_custom_css():
    st.markdown("""
    <style>
        /* 전체 배경색을 흰색이 아닌 차분한 연회색/베이지 톤으로 통일 */
        .stApp {
        ...
        .stTabs [data-baseweb="tab-list"] {
            gap: 10px;
            background-color: #e2e8f0; /* 탭 바 전체 배경 */
            padding: 10px;
            border-radius: 12px;
            border: 1px solid #cbd5e1; /* 외부 하이라이트 경계선 */
        }

        /* 기본 탭 버튼 스타일 */
        .stTabs [data-baseweb="tab"] {
            height: 50px;
            background-color: #94a3b8 !important; /* 기본 회색 배경 */
            color: #ffffff !important; /* 흰색 글씨 */
            border-radius: 8px !important;
            border: 1px solid #cbd5e1 !important; /* 탭 개별 테두리 */
        ...
        .stTabs [aria-selected="true"] {
            background-color: #1e293b !important; /* 진한 네이비/회색 */
            color: #ffffff !important;
            border: 2px solid #f1f5f9 !important; /* 활성 탭 외곽 하이라이트 */
...
# ==========================================
def main():
    st.set_page_config(page_title="RCS在庫관리", layout="centered")
    inject_custom_css()
    init_state()
    ...
    with tab3:
        ...
        with st.expander("➕ 新規アイテム登録"):
            ...
            n_cat_sel = st.selectbox("カテゴリ選択", options=exist_cats + ["(新規作成)"])
            final_cat = n_cat_sel if n_cat_sel != "(新規作成)" else st.text_input("新規カテゴリ名")
            col_a, col_b = st.columns(2)
            n_min = col_a.number_input("発注目安", min_value=0)
            n_unit = col_b.text_input("単位", value="個")
                ...
                with st.expander(f"📂 {cat}"):
                    ...
                    for _, row in cat_df.iterrows():
                        ...
                        if not st.session_state[ed_key]:
                            ...
                            if c3.button("削除", key=f"db_{i_id}"): supabase.table("inventory").delete().eq("id", i_id).execute(); del st.session_state.inventory_df; st.rerun()
                        else:
                            st.markdown("---")
                            ...
                            if st.button("保存", key=f"sb_{i_id}", type="primary"):
                            ...
                            if st.button("取消", key=f"cb_{i_id}"): st.session_state[ed_key] = False; st.rerun()
